...
- `consus_key_label` log field: the gateway's `x-consus-key-label` response
  header — the key's cosmetic, owner-chosen label. Display only: it is
  user-controlled text, so attribution stays on `consus_key_id`.
